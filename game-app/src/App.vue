<template>
  <div id="app" class="contain">
      <GameStart  v-if="gameState === 'start'">
        <p v-for="option in characterChoices" :key="option">
            <input v-model="characterSelected" type="radio" :id="option" name="optionSelected" :value="option" >
            <label :for="option">{{option}}</label>
        </p>
        <button @click="setCharacter">Select character</button>
      </GameStart>
...
export default {
  ...
  methods : {
    setCharacter(){
      this.$store.commit('setCharacter',this.characterSelected );
      this.$store.commit('setGameState',this.characterSelected ? 'characterSelected' : 'start');
    },
    selectAns(ans){
      this.$store.commit('selectAns', ans);
    }
  },
  computed : {
    ...mapState(['gameState','questions','character','questionIndex','characterChoices'])
  }
}
</script>
